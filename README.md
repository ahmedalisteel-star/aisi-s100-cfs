...
Section properties are not computed rigorously from raw geometry.
...
DSM requires elastic buckling values you supply. `Pcrl/Pcrd/Pcre`
...
Lateral-torsional buckling (EWM) needs an explicit Mcr. C and Z
...
Web crippling coefficients are not hard-coded. `C, C_R, C_N, C_h`
...
Shear phi/Omega split by h/t range was not independently verified
...
stamped submittal.
This is a design aid, not a substitute for a licensed engineer's
